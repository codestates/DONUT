...
module.exports = {
  ...
  detailFreetalk: async (req, res) => {
    const partFreetalk = await freetalk.findOne({where: {userId: req.body.userId, title: req.body.title}});

    if(!partFreetalk) {
      res.status(404).json({data: null, message: 'not found!'});
    } else {
      // 임시
      const freetalkInfo = {picture: partPost.picture, writing: partPost.writing};
